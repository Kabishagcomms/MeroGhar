    ...
    const submitAction = async () => {
      ...
      let kycdata: KycData = {
        kycInfo: {
          ...
          country:
            kycInfo!.country == formdata.country
              ? formdata.country
              : country.getCountryData(parseInt(formdata.country)).name,
          state:
            kycInfo!.state == formdata.state
              ? formdata.state
  ...
  return (
    <main key={'fuckU'} className="mt-5 w-full rounded-lg   p-4  ">
      ...
      <form>
        ...
        <div className="my-3 grid w-full grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          <div className="w-full">
            <label className="my-1 block text-sm font-semibold">Country </label>
            <select
              className={inputStyle}
              defaultValue={kycInfo!.country}
              {...register('country', { required: true })}
            >
              <option value={kycInfo!.country}>
                {kycInfo!.country == '' ? 'Select a Country' : kycInfo!.country}
              </option>
          ...
          <div className="w-full">
            ...
            >
              <option value={kycInfo!.state}>
                {kycInfo!.state == '' ? 'Select a State' : kycInfo!.state}
              </option>
              {country
          ...
          <div className="w-full">
            ...
            >
              <option value={kycInfo!.city}>
                {kycInfo!.city == '' ? 'Select a City' : kycInfo!.city}
              </option>
              {country
                .getCities(parseInt(watch('country')), parseInt(watch('state')))
                .map((city, index) => (
