...
export default function AccountComponent({
  ...
  return (
    <main className={`mx-auto rounded-lg ${bg}`}>
      <div className=" w-[97%] p-3 sm:w-[80%] ">
        {/* for kyc header */}
        ...
        {userMatch && !is_Admin && (
          <div className="my-6">
            <p className="text-md flex gap-x-1 font-semibold text-black">
              Status:{' '}
              <span className="flex items-center gap-x-1 ">
                ...
                {kyc?.isVerified && 'Verified'}
                {kyc!.pending && <AiFillHourglass className="h-5 w-5" />}{' '}
                {kyc?.pending == kyc!.isVerified && (
                  <RxCrossCircled className="h-5 w-5 " />
                )}
                {kyc?.isVerified && <AiFillCheckCircle className="h-5 w-5" />}
              </span>
            </p>
            ...
            {kyc!.message !== '' && (
              <p className="my-2 font-semibold">
                Message:{' '}
                <span className="text-sm text-red-500">{kyc!.message}</span>
              </p>
            )}
          </div>
        )}
