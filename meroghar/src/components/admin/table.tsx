...
export default function AdminTable({
  ...
  useEffect(() => {
    const onSearch = async () => {
      if (use == 'user') {
        return Api.get(`/admin/v1/allUsers/?search=${search}`, {
          withCredentials: true,
        })
          .then((res) => setStateUsers(res.data.users))
          .catch((e) => {
            setStateUsers(users)
          })
      }
      ...
      //for product search
      return Api.get(`/admin/v1/allProperties/?search=${search}`, {
        withCredentials: true,
      })
        .then((res) => setStateProperties(res.data.properties))
        .catch((e) => {
                    ...
                    return (
                      ...
                      >
                        <tr className="hover:bg-gray-100 dark:hover:bg-gray-700">
                          ...
                          <Link href={`/Home/user/${data._id}`} target="_space">
                            <td className="mr-12 flex items-center space-x-3 whitespace-nowrap p-4">
                              <Image
                                alt="ProfileImage"
                                height={48}
